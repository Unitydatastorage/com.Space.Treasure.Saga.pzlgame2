...
namespace Source.Level
{
    public class Level
    {
        ...
        private List<CellViewMono> _cellViews = new List<CellViewMono>();

        private List<CellViewMono> _mergedCellViews = new List<CellViewMono>();

        [Inject]
        private Level(GameStats gameStats)
        {
            _gameStats = gameStats;
            CellViewMono.OnSwapItem += CheckMergedCell;
        ...
        private void Generate()
        {
            ...
            {
                m_LevelViewMono.Transform.position = new Vector2(
                ...
                for (int i = 0; i < _gameSettingsScriptable.LevelSize.x; i++)
                {
                    for (int j = _gameSettingsScriptable.LevelSize.y; j > 0 ; j--)
                    {
                        ...
                        CellViewMono cellViewMono = m_CellsFactory.Create(position);
                        ItemMono itemMono = _itemFactory.Get();
                        cellViewMono.Init(itemMono);
                    
                        _cellViews.Add(cellViewMono);
                    }
                }
            }
        }
        ...
        private void CheckMergedCell(CellViewMono cellViewMono)
        {
            ...
            Vector2Int vector2Int = cellViewMono.Position;
            ...
            for (int i = vector2Int.x; i < _gameSettingsScriptable.LevelSize.x+1; i++)
            {
                ...
                if (findCell && !findCell.IsEmpty && !cellViewMono.IsEmpty && findCell.CurrentItem.index == cellViewMono.CurrentItem.index)
                {
        ...
        private void MoveItem()
        {
            foreach (var cellView in _cellViews)
            {
                ...
                movedPosition.y--;
                CellViewMono nextCellViewMono = _cellViews.FirstOrDefault(_ => movedPosition == _.Position);
                if (nextCellViewMono && nextCellViewMono.IsEmpty)
                {
