...
namespace Source.CellManagement.Mono
{
    public class CellViewMono : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler
    {
        ...
        private CellViewMono m_OldSwapCellView;

        public static Action<CellViewMono> OnSwapItem;
        ...
        private IEnumerator SwapCell(CellViewMono swapCellViewMono)
        {
            m_OldSwapCellView = swapCellViewMono;
            (_currentItem, swapCellViewMono._currentItem) = (swapCellViewMono._currentItem, _currentItem);
            _canSwap = false;
        ...
        {
            (_currentItem, m_OldSwapCellView._currentItem) = (m_OldSwapCellView._currentItem, _currentItem);
            _canSwap = false;
            yield return new WaitWhile(() => _currentItem.IsSwapping);
            yield return new WaitForSeconds(0.2f);
            _canSwap = true;
            m_OldSwapCellView = null;
        }
        ...
        {
            if (!_canSwap) return;
            if (eventData.pointerDrag && eventData.pointerDrag.TryGetComponent(out CellViewMono dragCellView))
            {
